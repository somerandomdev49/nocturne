...
namespace noct
{
	...
	{
		if(auto r = it_(name); !r.error) return r.value->second;
		else return nullptr;
	}
	...
	TypeRes ASTFunc::type(TypecheckEnv &env) const noexcept
	{
		...
		return { !decl.signature.returnType->assignable(t.value), decl.signature.returnType };
	}
	...
	{
		return { false, Ptr<TypeNumeric>(new TypeNumeric(getSuitableIntegerTypeFor(value))) };
	}
	...
	void ASTVar::print(std::ostream &out, int indent) const noexcept
	{
		...
		decl.type->print(out);
		if(value != nullptr) { out << " = "; value->print(out, 0); }
		out << ";";
	}
